...
class database(Extension):
	...
	def __init__(self, bot: commands.Bot):
		...
		if debugging: self.db: sqlite3.Connection = sqlite3.connect('./src/ext/database/testing.db')
		else: self.db: sqlite3.Connection = sqlite3.connect('./src/ext/database/main.db')
	...
	def add_new_user_from_id(self, _id):
		...
		self.cursor.execute(f"INSERT INTO Users VALUES ({values_str})", params)
		self.db.commit()
	def exist_check_from_id(self, _id):
		...
		"""
		try:
			self.cursor.execute("SELECT discord_id FROM Users WHERE discord_id=?", _id).fetchone()
		except:
	...
	async def new_member_in_db(self, ctx, user: discord.Member=None):
		...
		if self.cursor.execute("SELECT * FROM Users WHERE discord_id=?", (user.id,)).fetchone():
			await ctx.send(f"{ctx.author.mention}, user already in the database")
			return
		try: self.add_new_user_from_id(user.id)
	...
	@commands.command(name="backup", usage=f"{prefix}backup", brief="Starts a database backup")
	async def backup(self, ctx):
		"""
		Starts a database backup
		"""
		self.db.backup(sqlite3.connect('./src/ext/database/backups/backup.db'))
